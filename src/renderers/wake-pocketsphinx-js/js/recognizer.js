...
function startup( onMessage ) {
  self.onmessage = function ( event ) {
    var pocketsphinxJS = (event.data && event.data.length && (event.data.length > 0))
      ? event.data
      : 'pocketsphinx.js';
      ...
      post(
        {
          status: 'error',
          command: 'initialize',
          code: Module.ReturnType.RUNTIME_ERROR
        }
      );
    }
    else {
...
function addWords( data, clbId ) {
  if ( recognizer ) {
    ...
    for ( var i = 0; i < data.length; i++ ) {
      var w = data[ i ];
      if ( w.length == 2 ) {
        words.push_back( [ Utf8Encode( w[ 0 ] ), w[ 1 ] ] );
      }
    }
